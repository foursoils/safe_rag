...
class GraphExtractorMemory:
    ...
    def load(self):
        ...
        if os.path.exists(self.extractor_json_path):
            try:
                ...
                logger.info("loaded memory from %s", self.extractor_json_path)
            except Exception as e:
                logger.warning("could not load existing extractor memory: %s", e)
    ...
    def save(self):
        ...
        try:
            ...
            logger.warning("could not write GraphML: %s", e)
        try:
            data = nx.node_link_data(self.G)
            with open(self.extractor_json_path, 'w', encoding='utf-8') as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
            logger.info("saved extractor memory to %s", self.extractor_json_path)
        except Exception as e:
            logger.warning("could not write extractor JSON: %s", e)
