...
def load_original_graph_data(
    ...
    def _load_graphrag(path: str) -> Tuple[set, set, set, dict]:
        ...
        logger.info("Loading original GraphRAG data from %s dataset...", dataset_name)
        ...
        if "degree" in entities_df.columns:
            ...
            if filter_isolated:
                ...
                logger.info(
                    "Loaded %s connected entities (filtered out %s isolated)",
                    len(filtered_node_labels),
                    isolated_entities,
                )
                logger.info(
                    "Total original entities (including isolated): %s",
                    len(original_node_labels),
                )
            else:
                filtered_node_labels = original_node_labels
                logger.info(
                    "Loaded %s entities (including %s isolated)",
                    len(original_node_labels),
                    isolated_entities,
                )
        else:
            ...
            filtered_node_labels = original_node_labels
            logger.info(
                "Loaded %s entities (no degree information available)",
                len(original_node_labels),
            )
        ...
        logger.info("Loaded %s original relationships", len(original_edge_keys))
    ...
    def _load_lightrag(path: str) -> Tuple[set, set, set, dict]:
        ...
        logger.info("Loading original LightRAG data from %s dataset...", dataset_name)
        logger.info("GraphML file: %s", graphml_file)
        ...
        logger.info("Loaded %s original relationships (undirected)", len(original_edge_keys))
